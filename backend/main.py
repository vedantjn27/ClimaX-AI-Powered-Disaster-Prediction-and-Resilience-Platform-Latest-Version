...
# Force UTF-8 output on Windows to prevent cp1252 emoji encoding crashes
import sys
import io
...
class VoiceProcessor:
    """Creates a fresh pyttsx3 engine per call to avoid Windows COM reuse bug."""

    def _make_engine(self):
        """Initialise a brand-new engine each time to avoid the Windows COM
        'run loop already started' crash on subsequent calls."""
        engine = pyttsx3.init()
    ...
    def text_to_speech(self, text: str, language: str = "en", output_file: str = None, speed: int = 150):
        # Create a fresh engine every call — prevents Windows COM reuse crash
        engine = self._make_engine()
        try:
            engine.setProperty('rate', speed)

            # Try to pick a language-specific voice
            if language != "en":
                voices = engine.getProperty('voices')
                for voice in voices:
            ...
            if output_file:
                ...
                if os.path.exists(output_file) and os.path.getsize(output_file) > 0:
                    print(f"Audio file created: {output_file} ({os.path.getsize(output_file)} bytes)")
                    return True
                else:
                    print(f"Audio file empty or missing: {output_file}")
                    return False
            else:
                engine.say(text)
                engine.runAndWait()
            ...
            # Always stop the engine loop before discarding the object
            try:
                engine.stop()
            except Exception:
                pass
...
class DisasterResponseBot:
    def __init__(self):
        ...
        if not self.groq_api_key:
            raise ValueError("GROQ_API_KEY not found in environment variables! Get a free key at console.groq.com")

        if not self.weather_api_key:
            logger.warning("OPENWEATHER_API_KEY not configured — weather lookups will use mock data.")

        # Initialize Groq AI client (OpenAI-compatible)
        self.grok_client = make_ai_client(self.groq_api_key)
...
class GrokDisasterResilienceAnalyzer:
    def __init__(self):
    ...
    def configure_grok(self, api_key: str):
        """Configure Groq AI client"""
        try:
            self.client = make_ai_client(api_key)
            return True
        except Exception as e:
            logger.error(f"Failed to configure Grok API: {str(e)}")
            return False
    
    async def get_comprehensive_resilience_analysis(self, location: str, api_key: str) -> Dict:
        """Use Grok to fetch all relevant data and calculate resilience scores."""
        
        if not self.configure_grok(api_key):
            raise HTTPException(status_code=400, detail="Invalid Grok API key")
        # Try to get real AQI to stop hallucination
        real_aqi_context = "Unknown"
        try:
            # get_air_quality is an async endpoint function defined globally
            aqi_res = await get_air_quality(location)
            real_aqi_context = f"REAL-TIME AQI FROM WAQI is: {aqi_res.get('aqi')} (Air Quality Label: {aqi_res.get('aqi_label')})"
        except Exception:
            real_aqi_context = "Unavailable"
        ...
        *CRITICAL DATA INJECTION*: {real_aqi_context}. DO NOT hallucinate AQI values or air pollution statistics that conflict with this real data!
        ...
        **OUTPUT REQUIRED:**
        ...
        try:
            # Run Grok API call in thread pool to avoid blocking
...
---
*Report generated by Grok AI-powered Disaster Resilience Analyzer*
"""
        
        return report

# Global analyzer instance
analyzer = GrokDisasterResilienceAnalyzer()

# ==================== AGENTIC AI MODULE ====================
class ClimateAIAgent:
    def __init__(self, region: str, api_key: str = None):
        self.region = region
        self.client = make_ai_client(api_key or os.getenv("GROQ_API_KEY"))
    ...
    def get_region_data(self) -> Dict:
        ...
        region_info = {
            ...
            "Mumbai": {
                ...
                "major_issues": ["Flooding", "Sea level rise", "Urban heat island", "Air pollution"],
                "population": "21 million",
                "key_sectors": ["Financial services", "Entertainment", "Ports", "Manufacturing"],
                "current_aqi": 0,
                "avg_temp": 31,
                "rainfall_deficit": 20,
                "renewable_percent": 5.2,
                "waqi_city": "mumbai"
            },
            "Bangalore": {
                "climate_type": "Tropical savanna",
                "major_issues": ["Water crisis", "Lake pollution", "Urban flooding", "Heat stress"],
                "population": "13 million",
                "key_sectors": ["IT/Software", "Biotechnology", "Aerospace", "Electronics"],
                "current_aqi": 0,  # Will be replaced by live WAQI fetch
                "avg_temp": 29,
                "rainfall_deficit": 25,
                "renewable_percent": 14.5,
                "waqi_city": "bangalore"
            },
            "Odisha": {
                "climate_type": "Tropical monsoon",
                "major_issues": ["Cyclones", "Flooding", "Coastal erosion", "Drought in western districts"],
                "population": "46 million",
                "key_sectors": ["Mining", "Agriculture", "Steel", "Fisheries"],
                "current_aqi": 0,
                "avg_temp": 32,
                "rainfall_deficit": 20,
                "renewable_percent": 11.7,
                "waqi_city": "bhubaneswar"
            }
        }
    ...
    def get_mitigation_strategies(self) -> List[str]:
        ...
        strategies_map = {
            ...
            "Mumbai": [
                "Upgrade storm water drainage and sewer systems",
                "Implement mangrove restoration for coastal protection",
                "Develop flood-resilient infrastructure for low-lying areas",
                "Transition local transport to electric vehicles",
                "Create urban cooling centers and green roofs"
            ],
            "Bangalore": [
                "Restore and rejuvenate urban lakes and water bodies",
                "Implement rainwater harvesting mandates for all buildings",
                "Develop sustainable public transit (metro expansion)",
                "Create IT park green building standards",
                "Implement e-waste management and recycling programs"
            ],
            "Odisha": [
                "Strengthen cyclone early warning and shelter systems",
                "Build climate-resilient coastal infrastructure",
                "Promote drought-resistant crops in western districts",
                "Develop sustainable mining practices",
                "Implement mangrove and coastal forest restoration"
            ]
        }
...
def get_api_key():
    ...
    if not api_key:
        raise HTTPException(status_code=400, detail="GROQ_API_KEY not configured. Get a free key at console.groq.com")
    return api_key
        ...
        if self.groq_api_key:
            try:
                ...
                for pattern in disaster_patterns:
                    ...
                    prompt = f"""You are an expert climate policy advisor for India. Analyze the following and provide evidence-based policy recommendations:
...
{{
    "current_policies": ["list of 2-3 current relevant policies in {region} for {disaster_type}"],
    "evidence_based_recommendation": "A detailed 2-3 sentence evidence-based policy recommendation specific to {region} and {disaster_type}",
    "factors_considered": ["list of 3-4 regional factors that influence this recommendation"],
    "implementation_timeline": "realistic timeline like 6-12 months or 1-2 years",
    "budget_estimate": "estimated budget in INR crores",
    "expected_impact": "expected percentage reduction in disaster impact with brief explanation"
                    ...
                    try:
                        ...
                        policies.append({
                            ...
                            "current_policies": ai_result.get("current_policies", []),
                            "evidence_based_recommendation": ai_result.get("evidence_based_recommendation", ""),
                            "factors_considered": ai_result.get("factors_considered", []),
                            "implementation_timeline": ai_result.get("implementation_timeline", "6-12 months"),
                            "budget_estimate": ai_result.get("budget_estimate", "Not estimated"),
                            "expected_impact": ai_result.get("expected_impact", "Significant reduction expected")
                        })
                        ...
                        policies.append({
                            ...
                            "disaster_type": disaster_type,
                            "current_policies": [],
                            "evidence_based_recommendation": response_text,
                            "factors_considered": [f"AI analysis for {region}"],
                ...
                logger.error(f"Grok policy generation failed: {e}")
                # Fall through to basic fallback
        
        # Basic fallback if Grok is unavailable
        for pattern in disaster_patterns:
            region = pattern.get("region", "Unknown")
            disaster_type = pattern.get("disaster_type", "General")
            ...
            policies.append({
                "region": region,
                "disaster_type": disaster_type,
                "current_policies": ["Standard disaster preparedness protocols"],
                "evidence_based_recommendation": f"Implement comprehensive {disaster_type.lower()} management plan for {region} including early warning systems, community training, and infrastructure hardening.",
                "factors_considered": [f"Regional characteristics of {region}"],
                "implementation_timeline": "6-12 months",
                "budget_estimate": "To be assessed",
                "expected_impact": "Pending detailed analysis"
            })
...
class WeatherService:
    ...
    def _validate_city_name(self, city: str) -> str:
        """Validate and clean city name. Also accepts coordinate strings like '12.97,77.59' or URL-encoded '12.97%2C77.59'."""
        if not city or not city.strip():
            raise HTTPException(status_code=400, detail="City name cannot be empty")
        
        from urllib.parse import unquote
        cleaned_city = unquote(city.strip())  # Decode %2C -> comma etc.
        
        # Allow coordinate format: "lat,lon" (digits, dots, commas, minus)
        import re
        is_coord = re.match(r"^-?\d+(\.\d+)?,-?\d+(\.\d+)?$", cleaned_city)
        if is_coord:
            return cleaned_city
        
        # Regular city name: letters, spaces, hyphens, apostrophes
    ...
    async def _fetch_from_api(self, city: str) -> Dict[str, Any]:
        ...
        # Check if city string is actually coordinates (e.g. "12.97,77.59")
        if "," in city and any(c.isdigit() for c in city):
            try:
                lat, lon = city.split(",")
...
async def get_weather_forecast(city: str):
    """Get 5-day weather forecast for a city using OpenWeatherMap"""
    try:
        if not weather_service.api_key or weather_service.api_key == "demo_key":
            raise HTTPException(status_code=500, detail="OpenWeatherMap API key not configured")
        
        url = f"{weather_service.base_url}/forecast"
        # Support coordinate strings like "12.97,77.59"
        import re
        params: dict = {"appid": weather_service.api_key, "units": "metric"}
        if re.match(r"^-?\d+(\.\d+)?,-?\d+(\.\d+)?$", city.strip()):
            lat, lon = city.strip().split(",")
            params["lat"] = lat.strip()
            params["lon"] = lon.strip()
        else:
            params["q"] = city.strip()
        
        async with aiohttp.ClientSession(timeout=weather_service.timeout) as session:
            async with session.get(url, params=params) as response:
                if response.status != 200:
                    raise HTTPException(status_code=response.status, detail=f"Forecast API error")
...
@app.get("/weather/{city}/air-quality")
async def get_air_quality(city: str):
    """Get air quality data for a city using WAQI (or OWM fallback)"""
    try:
        if not weather_service.api_key or weather_service.api_key == "demo_key":
            raise HTTPException(status_code=500, detail="OpenWeatherMap API key not configured")
        ...
        if re.match(r"^-?\d+(\.\d+)?,-?\d+(\.\d+)?$", city.strip()):
            # Already coordinates
            lat_str, lon_str = city.strip().split(",")
            lat, lon = float(lat_str.strip()), float(lon_str.strip())
        else:
            # Geocode city name
            geo_url = f"http://api.openweathermap.org/geo/1.0/direct"
            geo_params = {"q": city.strip(), "limit": 1, "appid": weather_service.api_key}
        ...
        # Get air quality data from WAQI (World Air Quality Index Project)
        waqi_key = os.getenv("WAQI_API_KEY")
        if not waqi_key or waqi_key == "your_waqi_api_key_here":
            # Fallback to OpenWeatherMap with scaled IND-AQI if WAQI key is missing
            aqi_url = f"{weather_service.base_url}/air_pollution"
            aqi_params = {"lat": lat, "lon": lon, "appid": weather_service.api_key}
            
            async with aiohttp.ClientSession(timeout=weather_service.timeout) as session:
                async with session.get(aqi_url, params=aqi_params) as response:
                    if response.status != 200:
                        raise HTTPException(status_code=response.status, detail="AQI API error")
                    aqi_data = await response.json()
            
            aqi_item = aqi_data["list"][0]
            # OWM is 1-5, map to IND AQI approximate scale
            scale_mapping = {1: 45, 2: 95, 3: 155, 4: 250, 5: 400}
            aqi_value = scale_mapping.get(aqi_item["main"]["aqi"], 100)
            components = aqi_item["components"]
            ...
            aqi_params = {"token": waqi_key}
            
            async with aiohttp.ClientSession(timeout=weather_service.timeout) as session:
                async with session.get(aqi_url, params=aqi_params) as response:
                    if response.status != 200:
                        raise HTTPException(status_code=response.status, detail="WAQI API error")
                    waqi_data = await response.json()
            
            if waqi_data.get("status") != "ok":
                raise HTTPException(status_code=500, detail="WAQI data unavailable")
            ...
            comp_dict = {
                "pm2_5": round(iaqi.get("pm25", {}).get("v", 0), 1),
                ...
                "co": round(iaqi.get("co", {}).get("v", 0), 1)
            }

        # Map IND AQI scale to categories
        if aqi_value <= 50:
            aqi_label, aqi_color = "Good", "#4CAF50"
        elif aqi_value <= 100:
            aqi_label, aqi_color = "Satisfactory", "#8BC34A"
        elif aqi_value <= 200:
            aqi_label, aqi_color = "Moderate", "#FFEB3B"
        elif aqi_value <= 300:
            aqi_label, aqi_color = "Poor", "#FF9800"
        elif aqi_value <= 400:
            aqi_label, aqi_color = "Very Poor", "#F44336"
        else:
            aqi_label, aqi_color = "Severe", "#B71C1C"
        ...
        raise HTTPException(status_code=500, detail=f"AQI fetch failed: {str(e)}")

@app.post("/weather/alerts")
async def get_weather_alerts(data: dict):
    """Generate AI-powered weather alerts based on current conditions using Grok"""
    try:
        api_key = os.getenv("GROQ_API_KEY")
        if not api_key:
            return {"success": True, "alerts": [], "advisories": []}
        ...
        aqi = data.get("aqi", 0)
        aqi_label = data.get("aqi_label", "")

        client = make_ai_client(api_key)

        prompt = f"""Based on the current weather conditions in {city}, India, generate weather alerts and advisories.
...
{{
    "alerts": [
        {{"severity": "high/medium/low", "title": "Alert title", "message": "Brief alert message"}}
    ],
    "advisories": [
        "Advisory message 1",
        "Advisory message 2"
...
Only include alerts that are genuinely warranted by the conditions. Include 1-3 alerts and 2-4 advisories."""
        ...
        return {"success": True, "alerts": result.get("alerts", []), "advisories": result.get("advisories", [])}
        
    except Exception as e:
        logger.error(f"Weather alerts generation error: {e}")
        return {"success": True, "alerts": [], "advisories": []}

async def _fetch_waqi_aqi(city_name: str) -> int:
    """Fetch live AQI from WAQI API using geo-coordinates (same as weather tab).
    Geocodes city name via OWM, then queries WAQI geo:lat;lon feed for consistency."""
    waqi_key = os.getenv("WAQI_API_KEY")
    if not waqi_key or waqi_key == "your_waqi_api_key_here":
        return 0
    try:
        owm_key = weather_service.api_key
        # Step 1: Geocode city name to lat/lon via OWM
        geo_url = "http://api.openweathermap.org/geo/1.0/direct"
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=8)) as session:
            async with session.get(geo_url, params={"q": city_name, "limit": 1, "appid": owm_key}) as r:
                if r.status != 200:
                    return 0
            ...
            # Step 2: Query WAQI by geo coordinates (same endpoint as weather tab)
            waqi_url = f"https://api.waqi.info/feed/geo:{lat};{lon}/"
            async with session.get(waqi_url, params={"token": waqi_key}) as r2:
                if r2.status == 200:
                    data = await r2.json()
                    if data.get("status") == "ok":
                        return int(data["data"].get("aqi", 0))
    except Exception as e:
        logger.warning(f"WAQI geo fetch failed for {city_name}: {e}")
    return 0

@app.post("/region/{region}/full-analysis")
async def get_full_region_analysis(region: str, api_key: str = Depends(get_api_key)):
    """Generate comprehensive AI-driven analysis for a region covering all tabs"""
    try:
        agent = ClimateAIAgent(region, api_key)
        region_data = agent.region_data
        
        if not region_data:
            raise HTTPException(status_code=404, detail=f"Region '{region}' not found")
        
        # Fetch live AQI from WAQI and inject it into region_data
        waqi_city = region_data.get("waqi_city", region.lower())
        live_aqi = await _fetch_waqi_aqi(waqi_city)
        if live_aqi > 0:
            region_data["current_aqi"] = live_aqi
        
        prompt = f"""You are a climate analysis expert. Provide a comprehensive climate analysis for {region}, India.

Region Data:
- Climate Type: {region_data.get('climate_type', 'N/A')}
- Major Issues: {', '.join(region_data.get('major_issues', []))}
- Population: {region_data.get('population', 'N/A')}
- Key Sectors: {', '.join(region_data.get('key_sectors', []))}
...
{{
    "overview": {{
        ...
        "highlights": ["3-4 key highlights about the region's climate"]
    }},
    "climate_data": {{
        "temperature_trend": "Description of temperature trends over recent years",
        "rainfall_pattern": "Description of rainfall patterns",
        "extreme_events": ["List of 3-4 recent or likely extreme weather events"],
        "seasonal_analysis": "Brief seasonal climate analysis"
    }},
    "risk_assessment": {{
        "overall_risk": "High/Medium/Low",
        "primary_threats": ["Top 4-5 climate threats"],
        "vulnerability_factors": ["3-4 factors making the region vulnerable"],
        "projected_impacts": ["3-4 projected future impacts"]
    }},
    "mitigation": {{
        "immediate_actions": ["3-4 immediate actions needed"],
        "long_term_strategies": ["3-4 long-term strategies"],
        "adaptation_measures": ["3-4 adaptation measures"],
        "estimated_investment": "Estimated investment requirement"
    }}
}}"""
        ...
        # If JSON parsing fails, return a structured fallback using the raw text
        return {
            "success": True,
            "region": region,
            "region_data": region_data,
            "analysis": {
                "overview": {"summary": response_text[:500], "key_stats": [], "highlights": []},
                "climate_data": {"temperature_trend": "", "rainfall_pattern": "", "extreme_events": [], "seasonal_analysis": ""},
                "risk_assessment": {"overall_risk": "Medium", "primary_threats": region_data.get("major_issues", []), "vulnerability_factors": [], "projected_impacts": []},
                "mitigation": {"immediate_actions": [], "long_term_strategies": agent.get_mitigation_strategies(), "adaptation_measures": [], "estimated_investment": "To be determined"}
            },
            "generated_at": datetime.now().isoformat()
        }
    except Exception as e:
...
async def optimize_resources(request: Dict[str, Any]):
    ...
    # Return the result directly (no Pydantic model needed here)
    return optimization_result
...
@app.get("/bot/languages")
async def get_bot_languages():
    """Get list of supported languages for the disaster bot"""
    try:
        bot = get_bot()
        return {
...
async def bot_health_check():
    """Detailed health check for the disaster response bot"""
    try:
        bot = get_bot()
        return {
            "status": "healthy",
            "services": {
                "grok": "connected" if bot.grok_client else "disconnected",
...
async def get_regions():
    ...
    return {"regions": ["Delhi", "Mumbai", "Bangalore", "Uttarakhand", "Assam", "Gujarat", "Odisha", "Andhra Pradesh"]}
...
async def chat_with_agent(request: ChatRequest, api_key: str = Depends(get_api_key)):
    """Chat with the climate AI agent (agentic endpoint)"""
    try:
        region = getattr(request, 'region', 'Delhi')
        query = getattr(request, 'query', request.question)
        context_type = getattr(request, 'context_type', 'general')
        agent = ClimateAIAgent(region, api_key)
        response = agent.generate_response(query, context_type)
        return {
            "query": query,
            "response": response,
            "region": region,
...
def register_citizen(data: CitizenRegistration):
    ...
    # Generate RSA keypair and serialize both
    ...
    citizen_public_keys[data.citizen_id] = public_key_obj
...
def submit_alert(alert: AlertSubmission):
    ...
    # The frontend submits the private key in the signature field
    try:
        private_key = serialization.load_pem_private_key(
            alert.signature.encode('utf-8'),
            password=None
        )
        ...
        # Verify it against the org's public key to confirm ownership
        public_key.verify(
            signature_bytes,
            message_bytes,
            padding.PKCS1v15(),
        ...
        # Store the actual generated signature in base64, NOT the private key itself
        real_signature_b64 = base64.b64encode(signature_bytes).decode('utf-8')
    except Exception:
        raise HTTPException(status_code=400, detail="Invalid digital signature or private key")
...
def submit_feedback(feedback: Feedback):
    if feedback.citizen_id and feedback.signature:
        ...
        try:
            private_key = serialization.load_pem_private_key(
                feedback.signature.encode('utf-8'),
                password=None
            )
            signature_bytes = private_key.sign(
            ...
            feedback.signature = base64.b64encode(signature_bytes).decode('utf-8')
        except Exception:
            raise HTTPException(status_code=400, detail="Invalid signature or private key")
...
@app.post("/citizen/register")
def register_citizen(data: dict):
    citizen_id = data.get("citizen_id")
    name = data.get("name")
    if not citizen_id:
        raise HTTPException(status_code=400, detail="citizen_id is required")
    
    # CryptoUtils.generate_keypair() returns (public_pem, private_pem)
    public_key_pem, private_key_pem = CryptoUtils.generate_keypair()
    citizen_public_keys[citizen_id] = serialization.load_pem_public_key(public_key_pem.encode())
    citizens_db[citizen_id] = {"name": name, "public_key": public_key_pem}
    
    return {
        "message": f"Citizen {name} registered successfully", 
        "citizen_id": citizen_id,
...
@app.post("/organization/register")
def register_organization(data: dict):
    ...
    # CryptoUtils.generate_keypair() returns (public_pem, private_pem)
    public_key_pem, private_key_pem = CryptoUtils.generate_keypair()
    organizations_db[org_key] = {"name": org_name, "public_key": public_key_pem}
    
    return {
        "message": f"Organization {org_name} registered successfully", 
        "public_key": public_key_pem,
        "private_key": private_key_pem
    }

# Duplicate /optimize-resources removed — async version at line ~2962 is authoritative
...
def mine_block(new_data: NewBlockData):
    ...
    # Calculate and store hash so verify/status endpoints work correctly
    new_block.hash = new_block.calculate_hash()
    blockchain.chain.append(new_block)
    return {"message": "Block mined successfully", "block": {
...
def tamper_blockchain(tamper: TamperRequest):
    if 0 < tamper.block_index < len(blockchain.chain):
        block = blockchain.chain[tamper.block_index]
        block.transactions = tamper.new_data
...
# Resilience analyzer root (informational only — primary / is above)
@app.get("/resilience/info", response_model=StatusResponse)
async def resilience_info():
    """Resilience analyzer info endpoint"""
...
async def analyze_location(request: LocationAnalysisRequest):
    ...
    try:
        api_key = os.getenv("GROQ_API_KEY")
        if not api_key:
            raise HTTPException(status_code=500, detail="GROQ_API_KEY not configured. Get a free key at console.groq.com")
