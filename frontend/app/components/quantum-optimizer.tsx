...
interface ResourceMap { [key: string]: number }
...
export default function QuantumOptimizer({ backendConnected, backendUrl }: QuantumOptimizerProps) {
  ...
  const [demands, setDemands] = useState<Record<string, ResourceMap>>({
  ...
  const handleOptimize = async () => {
    ...
    try {
      if (backendConnected) {
        ...
        const totalDemand = Object.values(demands).reduce((sum: number, regionDemand: ResourceMap) => {
          return sum + Object.values(regionDemand).reduce((a: number, b: number) => a + b, 0)
        }, 0)
        const totalSupply = Object.values(resources).reduce((a: number, b: number) => a + b, 0)
        ...
        regions.forEach((region) => {
          const regionDemand: ResourceMap = demands[region] || {}
          const allocationRatio = Math.min(totalSupply / totalDemand, 1.0)

          mockResult.allocation[region] = {}
          Object.entries(regionDemand).forEach(([resource, demand]: [string, number]) => {
