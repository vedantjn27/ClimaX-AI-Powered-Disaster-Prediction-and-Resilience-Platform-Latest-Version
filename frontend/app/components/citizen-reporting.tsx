...
export default function CitizenReporting({ backendConnected, backendUrl }: CitizenReportingProps) {
  ...
  const handleDetectLocation = async () => {
    ...
    try {
      ...
      setReportForm((prev) => ({ ...prev, location: { lat: latitude, lon: longitude } }))
      ...
      alert("Could not detect location. Please ensure location permissions are enabled.")
    } finally {
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <div className="flex items-center space-x-2">
                ...
                <Input
                  placeholder="Click Detect to auto-detect your location"
                  value={locationName || (reportForm.location.lat !== 0 ? `${reportForm.location.lat.toFixed(4)}, ${reportForm.location.lon.toFixed(4)}` : "")}
                  readOnly
                />
                <Button variant="outline" size="sm" onClick={handleDetectLocation} disabled={isDetecting} className="bg-transparent">
                  {isDetecting ? "Detecting..." : "📍 Detect"}
                </Button>
              </div>
              {locationName && (
                <p className="text-xs text-green-600 mt-1">✓ Location detected successfully</p>
              )}
            </div>
